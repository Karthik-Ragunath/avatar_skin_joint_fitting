...
def parse_arguments():
    ...
    parser.add_argument("--source_directories", "-src_dirs", help="specify source directories", type=str, nargs="+", required=True)
    parser.add_argument("--target_directories", "-tar_dirs", help="specify target directories", type=str, nargs="+", required=True)
...
if __name__ == "__main__":
    ...
    source_file_dirs = args.source_directories
    target_file_dirs = args.target_directories
    num_inference_meshes = args.num_inference_meshes
    source_files = []
    target_files = []
    for source_file_dir, target_file_dir in zip(source_file_dirs, target_file_dirs):
        source_dir_files = sorted(glob.glob(os.path.join(source_file_dir, "*.obj")))
        target_dir_files = sorted(glob.glob(os.path.join(target_file_dir, "*.obj")))
        source_dir_files = source_dir_files[:(-1 * num_inference_meshes)]
        target_dir_files = target_dir_files[:(-1 * num_inference_meshes)]
        source_files.extend(source_dir_files)
        target_files.extend(target_dir_files)
