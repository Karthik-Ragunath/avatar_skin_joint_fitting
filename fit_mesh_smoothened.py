...
def parse_arguments():
    ...
    parser.add_argument("--model_type", "-m_type", help="provide the type of the trained model used", type=str, required=False, default="without_joints_smoothened")
...
def feed_auto_encoder(pose_auto_encoder: MultiFramePoseAE, ground_truth: torch.Tensor, condition: torch.Tensor, future_weights: torch.Tensor, faces: torch.Tensor, device: str):
    ...
    mesh = Meshes([vae_output], [faces.to(device)]).to(
        device
    )
    laplacian_mesh_loss = mesh_laplacian_smoothing(mesh)
    ...
    return (vae_output, mu_prior, logvar_prior), (recon_loss, laplacian_mesh_loss)
...
def main(args: SimpleNamespace, source_mesh_file_paths: List, target_mesh_file_paths: List):
    ...
    pose_auto_encoder.train()

    auto_encoder_optimizer = optim.Adam(pose_auto_encoder.parameters(), lr=args.initial_lr)
    ...
    for epoch in range(1, num_epochs + 1):
        ep_loss = 0
        mesh_index = 0 # for maintaining coding practice, since mesh_index will be used down the line for division
        for mesh_index, (source_mesh_file_path, target_mesh_file_path) in enumerate(zip(source_mesh_file_paths, target_mesh_file_paths)):
            ...
            for mini_batch_index, indices in enumerate(sampler):
                ...
                (vae_output, _, _), (recon_loss, laplacian_smoothing_loss) = feed_auto_encoder(
                    pose_auto_encoder, ground_truth.clone(), condition.clone(), future_weights, source_faces, args.device
                )
                auto_encoder_optimizer.zero_grad()
                (recon_loss + laplacian_smoothing_loss).backward()
                auto_encoder_optimizer.step()
                ep_loss += (float(recon_loss) + float(laplacian_smoothing_loss))
                logger.info(f'epoch - {epoch}; mesh_index: {mesh_index}; mini_batch_index: {mini_batch_index} - recon_loss: {float(recon_loss)}; smoothing_loss: {float(laplacian_smoothing_loss)}')

        avg_ep_loss = ep_loss / ((mini_batch_index + 1) * (mesh_index + 1))
        logger.info(f"Average Loss for epoch: {epoch} is {avg_ep_loss}")
